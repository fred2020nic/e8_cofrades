...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title">Motivo Baja</h3>
		<div class="card-tools">
			<a href="javascript:void(0)" id="create_new" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span> Crear Motivo Baja</a>
		</div>
	</div>
	<div class="card-body">
			...
			<div class="container-fluid">
				<table class="table table-bordered table-stripped">
					...
					<tbody>
						<?php
						$i = 1;
						$qry = $conn->query("SELECT * from `motivo_baja` where delete_flag= 0 order by `des_baja` asc ");
...
<script>
	$(document).ready(function() {
		$('.delete_data').click(function() {
			_conf("¿Estás seguro de eliminar esta baja de forma permanente?", "delete_brand", [$(this).attr('data-id')])
		})
		$('#create_new').click(function() {
			uni_modal("Agregar Banco", "cofrades_aux/manage_baja.php", "mid-large")
		})
	...
	function delete_brand($id) {
		start_loader();
		$.ajax({
			url: _base_url_ + "classes/Master.php?f=delete_baja",
			method: "POST",
			data: {
				id: $id
			},
